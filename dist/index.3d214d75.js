...
const showDeleteBtn = (id)=>{
    ...
    deleteBtn.addEventListener("click", _.throttle(()=>{
        ...
        if (document.getElementById(id).id < 26) (0, _notiflixDefault.default).Notify.info("Elementy początkowe nie mogą być usunięte permamentnie z listy. Usuwanie ich może doprowadzić do duplikacji");
        listEl.removeChild(document.getElementById(id));
        if (deletedDrinksArr.length > 0) deletedDrinksBtn.style.display = "block";
    }, 300));
};
const hideDeleteBtn = (id)=>{
    const deleteBtn = document.getElementById(id).childNodes[1];
    if (screen.availWidth > 1280) deleteBtn.style.display = "none";
};
const showModal = ()=>{
...
const addElement = ()=>{
    ...
    const newDrink = listEl.insertAdjacentHTML("beforeend", `
	<li
							id=${idCounter}
    ...
    hideModal();
    handleDeleteBtn();
    (0, _notiflixDefault.default).Notify.success("Pomyślnie dodane nap\xf3j");
};
const hideModal = ()=>{
...
const showDeletedDrinks = ()=>{
    ...
    saveBtns.forEach((el)=>el.addEventListener("click", (e)=>{
...
const saveElement = (id)=>{
    ...
    const savingEl = listEl.insertAdjacentHTML("beforeend", `
	<li
							id=${id}
...
const deleteElement = (id)=>{
    ...
    document.addEventListener("click", (e)=>{
        if (e.target === deleteCancelBtn) deletePopupEl.style.display = "none";
            ...
            deletePopupEl.style.display = "none";
            if (+drinkToDelete.id > 25) (0, _notiflixDefault.default).Notify.success("Usunięto permamentnie.");
        } else if (e.target === modalShadowEl) deletePopupEl.style.display = "none";
    });
    if (screen.availWidth < 1280) {
        const deleteBtn = document.getElementById(id).childNodes[1];
        deleteBtn.style.display = "block";
...
const search = (e)=>{
    ...
    drinksEl.forEach((item)=>{
        if (item.textContent.toLowerCase().includes(text)) return item.style.display = "flex";
        return item.style.display = "none";
    });
};
const updateList = ()=>{
    if (localStorage.getItem("savedDrinks")) {
        JSON.parse(localStorage.getItem("savedDrinks")).forEach((el)=>storagedDrinks.push(el));
        JSON.parse(localStorage.getItem("savedDrinks")).forEach((el)=>listEl.insertAdjacentHTML("beforeend", `<li
							id=${el.id}
    ...
    if (localStorage.getItem("drinksToDelete")) JSON.parse(localStorage.getItem("drinksToDelete")).forEach((el)=>deletedDrinksArr.push(el));
    if (deletedDrinksArr.length) deletedDrinksBtn.style.display = "block";
...
clearStorageBtn.addEventListener("click", ()=>{
    deletePopupTextEl.textContent = "Usunąć pamięć lokalną i zresetować aplikację?";
    deletePopupEl.style.display = "flex";
    document.addEventListener("click", (e)=>{
        if (e.target === deletePermamentBtn) {
            localStorage.clear();
            deletePopupEl.style.display = "none";
            (0, _notiflixDefault.default).Notify.success("Pamięć lokalna usunięta. Strona zostanie przywr\xf3cona do stanu pierwotnego po odświerzeniu.");
        } else if (e.target === deleteCancelBtn) deletePopupEl.style.display = "none";
    });
});
const handleDeleteBtn = ()=>{
    ...
    listItemEl.forEach((el)=>{
        el.addEventListener("mouseover", (e)=>{
            ...
            if (e.target.nodeName !== "I") showDescription(+e.target.id);
        });
    });
};
updateList();
...
},{}]},["8BXtR","bB7Pu"], "bB7Pu", "parcelRequire94c2")
